defmodule Aerolinea do
  ...
  def main do
    ...
    case parsear_destino(destino_input) do
      ...
      {:ok, destino} ->
        silla  = "Desea seleccionar silla? +$15.000 [s/n]: " |> Util.ingresar(:texto) |> opcion_bool()
        maleta = "Desea maleta de bodega?  +$45.000 [s/n]: " |> Util.ingresar(:texto) |> opcion_bool()
        seguro = "Desea seguro de viaje?   +$12.000 [s/n]: " |> Util.ingresar(:texto) |> opcion_bool()
  ...
  def calcular_total(destino, silla, maleta, seguro) do
    ...
    total = total = tarifa_base(destino) + cargo_silla(silla) + cargo_maleta(maleta_final) + cargo_seguro(seguro)
